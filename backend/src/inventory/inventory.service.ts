...
export class InventoryService {
  ...
  async update(id: number, payload: CreateInventoryDto) {
    ...
    Object.assign(entry, {
      ...
      payment_status,
      date: new Date(payload.date),
      pickup_date: !isOpening && payload.pickup_date ? new Date(payload.pickup_date) : null,
      delivery_date: !isOpening && payload.delivery_date ? new Date(payload.delivery_date) : null,
  ...
  async stockSummary() {
    const receivedRows = await this.inventoryRepo
      ...
      .getRawMany<{ product_id: number; cement_brand_id: number | null; received: string; received_value: string }>();
    ...
    const soldRows = await this.saleItemsRepo
      ...
      .getRawMany<{ product_id: number; cement_brand_id: number | null; sold: string }>();

    const rowKey = (productId: number, brandId?: number | null) => `${productId}-${brandId ?? 0}`;

    const soldMap = new Map<string, number>(
      soldRows.map((row) => [rowKey(Number(row.product_id), row.cement_brand_id), Number(row.sold)]),
    );
    ...
    details.forEach((entry) => {
      ...
      if (!latestPricedByKey.has(key) && Number(entry.purchase_price_per_unit) > 0) {
    ...
    return receivedRows.map((row) => {
      ...
      const brandId = row.cement_brand_id == null ? null : Number(row.cement_brand_id);
      ...
      const unit_price = Number(latestPriced?.purchase_price_per_unit ?? fallbackUnitPrice);
